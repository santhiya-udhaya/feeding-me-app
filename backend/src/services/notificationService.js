...
export async function createNotification({ recipient, type, title, message, relatedDonation, relatedPickup, relatedNGO }) {
  ...
  const duplicate = await Notification.exists({ userId: recipient, type, relatedDonationId: relatedDonation || null, relatedPickupId: relatedPickup || null, relatedNGOId: relatedNGO || null });
